...
outwd=$allwd/3-remove_marker-reverse
...
temp_file=temp_file
mkdir -p temp_folder
filename=$allwd/2-remove_marker/${base_filename}/temp_file666

i=1
cat $seq_p50_reverse | while read seq_i
do
 if [ $i -eq 1 ];then
...
echo ---------------------$i------------------------
mv ./temp_folder/${temp_file}666 ./${base_filename}.cut_reverse.fqt
...
cd $pwd
#cd /root/data/2-remove_marker/C
done